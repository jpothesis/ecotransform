...
// ✅ IMPORTS (Correct & Working with Vite)
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div>
            <h4 className="text-2xl font-extrabold mb-3 border-b border-white/50 pb-2 text-white">
              {data.title}
            </h4>
...
function CommunityPost({ post }) {
  ...
  return (
    <div
      className={`bg-white rounded-xl shadow-lg hover:shadow-xl transition-shadow overflow-hidden border ${
        post.isFeatured ? "border-orange-300 lg:col-span-2" : "border-amber-100"
      }`}
    >
      <div className="p-6">
        {post.isFeatured && (
        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs font-medium text-orange-600 bg-orange-100 px-2 py-0.5 rounded-full inline-block mt-0.5">
              {post.role}
            </p>
        ...
        {post.image && (
          <img
            src={post.image}
            alt={post.author}
            className="w-full h-auto max-h-96 object-cover rounded-lg mb-4 border border-amber-100"
          />
        )}
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {post.tags.map((tag, index) => (
            <span
              key={index}
              className="text-xs text-orange-700 bg-orange-50 px-3 py-1 rounded-full flex items-center gap-1"
            >
      ...
      <div className="max-w-7xl mx-auto px-6 py-20">
        ...
        <div className="text-center mb-16">
          <span className="text-sm font-semibold text-orange-600 bg-orange-100 px-4 py-2 rounded-full">
            🌱 Artisan & Contributor Hub
          </span>
          <h1 className="text-5xl font-extrabold text-amber-950 mt-4">
            Connect & Collaborate
          </h1>
          <p className="text-xl text-amber-800 max-w-3xl mx-auto mt-4">
            Share your sustainable creations, connect with fellow artisans, and
            celebrate transformations that matter.
          </p>
        </div>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="bg-white rounded-xl shadow-xl p-6 mb-8 border border-orange-200">
              ...
              <div className="flex gap-4 items-start mb-4">
                ...
                />
                <textarea
                  placeholder="Share your latest zero-waste project..."
                  rows="4"
                  className="flex-1 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-amber-900 resize-none"
                ></textarea>
              </div>
            ...
            <div className="sticky top-20 bg-white p-6 rounded-xl shadow-lg border border-orange-200">
              ...
              <ul className="space-y-4">
                {trendingTopics.map((topic, index) => (
                  <li
                    key={index}
                    className="pb-4 border-b border-amber-100 cursor-pointer"
                  >
